  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Panel - Project Selection & Params */}
            ...
            <CardContent className="space-y-5">
              ...
              <div>
                <label className="text-xs font-medium mb-2 block">
                  Variação de Preço: ±{priceVar}%
                </label>
                <Slider
                  value={[priceVar]}
                  onValueChange={([v]) => { setPriceVar(v); setSimulationResult(null); }}
                  min={5} max={40} step={1}
                />
              </div>
              ...
              <div>
                <label className="text-xs font-medium mb-2 block">
                  Variação de Demanda: ±{demandVar}%
                </label>
                <Slider
                  value={[demandVar]}
                  onValueChange={([v]) => { setDemandVar(v); setSimulationResult(null); }}
                  min={5} max={50} step={1}
                />
              </div>
              ...
              <Button
                className="w-full"
                disabled={!selectedProjectIds.length || loadingData || isSimulating}
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <SummaryCard
                    icon={DollarSign}
                    label="Receita Média"
                    value={fmt(simulationResult.summary.avgRevenue)}
                  />
                  <SummaryCard
                    icon={TrendingUp}
                    label="Lucro Médio"
                    value={fmt(simulationResult.summary.avgProfit)}
                    positive={simulationResult.summary.avgProfit > 0}
                  />
                  <SummaryCard
                    icon={Target}
                    label="ROI Médio"
                    value={fmtPct(simulationResult.summary.avgROI)}
                    positive={simulationResult.summary.avgROI > 0}
                  />
                  <SummaryCard
                    icon={Zap}
                    label="Prob. Sucesso"
                    value={fmtPct(simulationResult.summary.successProbability)}
                    positive={simulationResult.summary.successProbability > 50}
                  />
                </div>
                        ...
                        <BarChart data={scenarioChartData}>
                          ...
                          <Bar dataKey="receita" name="Receita" radius={[4, 4, 0, 0]}>
                            {scenarioChartData.map((entry) => (
                              <Cell key={entry.key} fill={scenarioColors[entry.key]} />
                            ))}
                          </Bar>
                      ...
                      <CardContent className="pt-4 pl-5">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div>
                            ...
                            <p className="font-semibold">{fmt(s.revenue)}</p>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Lucro</span>
                            <p className={`font-semibold ${s.profit >= 0 ? "text-green-500" : "text-destructive"}`}>
                              {fmt(s.profit)}
                            </p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className={`font-semibold ${s.roi >= 0 ? "text-green-500" : "text-destructive"}`}>
                              {fmtPct(s.roi)}
                            </p>
                          </div>
                        </div>
                      ...
                      {simulationResult.sensitivityMatrix.map((s, i) => (
                        <div key={i} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-6 bg-muted/30 rounded-full relative overflow-hidden">
                            <div
                              className="absolute left-1/2 h-full rounded-full transition-all"
                              style={{
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                              <span className="text-[10px] font-semibold">
                                {s.impactOnRevenue >= 0 ? "+" : ""}{s.impactOnRevenue.toFixed(1)}% receita
                              </span>
                            </div>
                          </div>
                        </div>
